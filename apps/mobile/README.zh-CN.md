...
语音输入使用 DSH 页面的 `getUserMedia`，不经过扩展 Bridge。Android App 首次使用时申请麦克风权限，仅向已配对 HTTPS Origin 的纯音频请求授权；相机和其他 Origin 仍被拒绝。同一 DSH 页面中的客户端插件共享 Origin，因此请只在信任已安装插件时启用语音输入。
