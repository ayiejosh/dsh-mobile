...
| [接入你既有的 frps](ATTACH_EXISTING_FRPS.md) | [Attach to an existing frps](ATTACH_EXISTING_FRPS.en.md) | 0.4.6 候选版：复用 VPS 上已运行的 frps，不自动修改服务器；可选公开 CA + Caddy 或自签 CA + TCP 透传，须核对监听与证书有效期。 |
...
> 既有 frps 接入及自签入口属于尚未发布的 0.4.6 候选版；线上 0.4.5 插件和 App 均不提供该入口。自签档须等待 0.4.6 Android App，旧版不能配对。
